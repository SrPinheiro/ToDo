...
type Props = {
  ...
  setPostEdit: React.Dispatch<
    React.SetStateAction<postType| null>
  >;
};

const Modal = ({ postData, setPostEdit }: Props) => {
  const modalRef = useRef<HTMLDivElement>(null);
  const [postEditado, setPostEditado] = useState<postType>({
    title: "",
    difficulty: 0,
  });
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (
      posts
        ...
        .some((post) => post.title === postEditado.title)
    ) {
  ...
  return (
    <div
      id="modal"
      className={`${styles.disactive} ${styles.container}`}
      ref={modalRef}
    >
